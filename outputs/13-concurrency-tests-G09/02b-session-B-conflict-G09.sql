...
-- File: outputs/13-concurrency-tests-G09/02b-session-B-conflict-G09.sql
-- Purpose: Session B of the conflicting-approval test. Approves the SECOND
--          pending booking of the overlap pair via dbo.sp_approve_booking.
-- Run together with 02a-session-A-conflict-G09.sql (start both ~simultaneously).
-- Expected: Session B fails with Msg 50000 — overlapping approved booking
--           already exists for this space during the requested time period.
-- ============================================================================
...
EXEC dbo.sp_approve_booking
    ...
    @decision_note     = 'Concurrent approval (Session B).';

PRINT 'Session B: approval SUCCEEDED (unexpected!).';
GO
